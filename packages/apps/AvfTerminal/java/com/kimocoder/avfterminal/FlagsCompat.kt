 ...
 *
 * This wrapper catches [NoClassDefFoundError] and returns safe defaults (false), disabling the
 * gated features rather than crashing.
 */
object FlagsCompat {

    private const val TAG = "FlagsCompat"

    /** Cached results to avoid repeated reflection/exception overhead. */
    ...
    fun terminalGuiSupport(): Boolean {
        ...
        return try {
            val result = com.android.system.virtualmachine.flags.Flags.terminalGuiSupport()
            cachedGuiSupport = result
            result
        } catch (e: NoClassDefFoundError) {
            Log.w(TAG, "Flags class not available on this device, disabling terminalGuiSupport", e)
            cachedGuiSupport = false
            false
        }
    }
    ...
    fun terminalStorageBalloon(): Boolean {
        ...
        return try {
            val result = com.android.system.virtualmachine.flags.Flags.terminalStorageBalloon()
            cachedStorageBalloon = result
            result
        } catch (e: NoClassDefFoundError) {
            Log.w(TAG, "Flags class not available on this device, disabling terminalStorageBalloon", e)
            cachedStorageBalloon = false
            false
        }
    }
}
